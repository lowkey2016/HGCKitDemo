...
NSString *const kYMUReachabilityChangedNotification = @"kYMUReachabilityChangedNotification";

@interface YMUReachability ()
...
#pragma unused (target)
    ...
    YMUReachability *reachability = ((__bridge YMUReachability*)info);
#else
    YMUReachability *reachability = ((YMUReachability*)info);
#endif
...
@implementation YMUReachability
...
{
    return [YMUReachability reachabilityWithHostname:hostname];
}
...
-(YMUNetworkStatus)currentReachabilityStatus
{
    if([self isReachable])
    {
        if([self isReachableViaWiFi])
            return YMUReachableViaWiFi;
        
#if	TARGET_OS_IPHONE
        return YMUReachableViaWWAN;
#endif
    }
    
    return YMUNotReachable;
}
...
{
	YMUNetworkStatus temp = [self currentReachabilityStatus];
...
-(void)reachabilityChanged:(SCNetworkReachabilityFlags)flags
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:kYMUReachabilityChangedNotification
                                                            object:self];
    });
}
...
- (void)printReachabilityString {
    NSLog(@"%@", [self currentReachabilityString]);
}
